...
setupJavaApplicationAlternatives()
{
	...
	--install /usr/bin/java java $jdkSymbolicPath/bin/java $jvmPriority \
	...
	--slave $libPath/jvm-exports/jre jre_exports $jvmExportsPath \
	\
...
setupJavaCompilerAlternatives()
{
	...
	sudo /usr/sbin/update-alternatives \
	--install /usr/bin/javac javac $jdkSymbolicPath/bin/javac $jvmPriority \
...
setupBrowserPluginAlternatives()
{
	...
	sudo /usr/sbin/update-alternatives \
	--install $libPath/browser-plugins/javaplugin.so javaplugin $jdkSymbolicPath/jre/lib/$arch/libnpjp2.so $jvmPriority \
...
startInstallation()
{
	...
	local installPath="/usr/local/jvm"
	local jvmPriority=777
	...
	local jdkAbsolutePath="$installPath/$tarArchiveName"
	...
	copyToLibraryPath "$tarArchiveName" "$installPath" || 
		exitWithMessage "Could not copy library to $installPath"
		
	local javaFullVersion=$(getJavaFullVersion "$jdkAbsolutePath")
	local javaVersion=$(getJavaVersion "$jdkAbsolutePath")
	local jdkSymbolicPath="$installPath/$javaVersion"
	local jvmExportsPath="$installPath/jvm-exports/$javaVersion"
	...
	setupJavaApplicationAlternatives "$jdkSymbolicPath" "$libPath" "$jvmExportsPath" "$jvmPriority" ||
		exitWithMessage "Failed updating Application alternatives"
	
	setupJavaCompilerAlternatives "$libPath" "$jdkSymbolicPath" "$jvmExportsPath" "$jvmPriority" ||
		exitWithMessage "Failed updating Compiler alternatives"

	setupBrowserPluginAlternatives "$arch" "$jdkSymbolicPath" "$libPath" "$jvmPriority" ||
