...
echo "#     ./shadowsocks-go.sh 2>&1 | tee shadowsocks-go.log     #"
echo "# Uninstall:                                                #"
echo "#     ./shadowsocks-go.sh uninstall                         #"
echo "# Upgrade:                                                  #"
echo "#     ./shadowsocks-go.sh uninstall                         #"
echo "#     ./shadowsocks-go.sh 2>&1 | tee shadowsocks-go.log     #"
echo "#############################################################"
...
function pre_install(){
    ...
    read -p "(Default password: qazwsx123):" shadowsockspwd
    [ -z "$shadowsockspwd" ] && shadowsockspwd="qazwsx123"
    echo ""
    echo "---------------------------"
...
function download_files(){
    cd $cur_dir
    if is_64bit; then
        if ! wget -c http://dl.chenyufei.info/shadowsocks/1.1.4/shadowsocks-server-linux64-1.1.4.gz;then
        ...
        if ! wget -c http://dl.chenyufei.info/shadowsocks/1.1.4/shadowsocks-server-linux32-1.1.4.gz;then
    ...
    if [ "$OS" == 'CentOS' ];then
        if ! wget --no-check-certificate -O shadowsocks-go https://github.com/hwangjr/scripts/raw/master/shadowsocks/shadowsocks-go; then
            echo "Failed to download shadowsocks-go auto start script!"
            exit 1
        fi
    else
        if ! wget --no-check-certificate -O shadowsocks-go https://github.com/hwangjr/scripts/raw/master/shadowsocks/shadowsocks-go-debian; then
            echo "Failed to download shadowsocks-go auto start script!"
            exit 1
        fi
    fi
